...
def mltp_3point(path, duration):
    ...
    path_transpose = np.array(path).T # tranpose ? 
    print("This is path transpose", path_transpose )
    ...
    while dtime<duration:
        ...
        # trajectory 
        for i in range(3):
            last_position[i] = _trajectory_3point(dtime/duration, path_transpose[i])
            # if i == 2:
    ...
    with open('time_history_' + 'higherorder' + '.npy', 'wb') as file:
        np.save(file, time_history)
    with open('theta_history_' + 'higherorder' + '.npy', 'wb') as file:
        np.save(file, theta_history)
    with open('ee_history_' + 'higherorder' + '.npy', 'wb') as file:
        np.save(file, ee_history)
    with open('angular_speed_history_' + 'higherorder' + '.npy', 'wb') as file:
        np.save(file, angular_speed_history)



# points = [q0, q1, q2] and not normalized
def _trajectory_3point(dtime, points):
...
def PID_goto_shit(last_position, duration):
    ...
    while dtime<duration:
        ...
        forward = Forward(Position_absolute_read(1), Position_absolute_read(2), Position_absolute_read(3))[1:4]

        some_value = 2 

        first_element = ((forward[0] > last_position[0] - some_value) or (forward[0] < last_position[0] + some_value))
        second_element = ((forward[1] > last_position[1] - some_value) or (forward[0] < last_position[1] + some_value))
        third_element = ((forward[2] > last_position[2] - some_value) or (forward[2] < last_position[2] + some_value))

        if first_element and second_element and third_element:
            Motion_z_endeffector(0)
